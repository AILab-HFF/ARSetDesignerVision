...
private void SpawnGLBModel(string modelPath, string modelName)
{
    ...
    // Optionaler Parent: erst Views, sonst XR Origin, sonst kein Parent
    Transform parent = null;
    var views = GameObject.Find("-----Views-----");
    if (views != null) parent = views.transform;
    else
    {
        var xrOrigin = GameObject.Find("XR Origin");
        if (xrOrigin != null) parent = xrOrigin.transform;
    }
    ...
    if (parent != null)
            meshyObject.transform.SetParent(parent, true); // Weltpos beibehalten

    // Platzierung: Pose vom Ray (falls vorhanden), sonst 1m vor der Kamera
    ...
    // (Optional) dein Materialize-Material anwenden, falls vorhanden
